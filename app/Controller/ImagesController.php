            ...
            if ($this->isAjax()) {
                ...
                if (isset($_POST['screenshot']) && isset($_POST['effect']) && isset($_POST['position'])) {
                    ...
                    if ($screenshotImage !== false || $effectImage !== false) {
                        imagecolortransparent($effectImage,imagecolorat($effectImage,0,0));
                        imagecopymerge($screenshotImage, $effectImage, intval($positionPost[0]), intval($positionPost[1]), 0, 0, imagesx($effectImage), imagesy($effectImage), 60);
                        $userId = $session->read('auth');
